                        ...
                        <div class="row w-100 align-items-center">
                            <div class="col-md-4 text-center d-none d-md-block position-relative">
                                <img src="<?php echo htmlspecialchars($imgPortadaDest); ?>"
                                    class="rounded-4 shadow-lg position-relative z-2 hero-img-destacada" alt="Portada">
                            </div>
                            <div class="col-md-8 text-white p-4">
                                <div class="d-flex flex-wrap gap-2 mb-3">
                                    <span class="badge bg-warning text-dark text-uppercase fw-bold"><i
                                            class="fas fa-fire me-1"></i> Destacado</span>
                                ...
                                <h1 class="display-4 fw-bold mb-2 text-shadow"><?php echo htmlspecialchars($obra['titulo']); ?>
                                </h1>
                                <div class="d-flex align-items-center mb-3">
                                    ...
                                    <span class="ms-3 text-light opacity-75"><i class="fas fa-pen-nib me-1"></i>
                                ...
                                <a href="/obra/<?php echo urlencode($obra['slug']); ?>"
                                    class="btn btn-iori btn-lg rounded-pill px-5 shadow-sm fw-bold border-0">
...
<main class="container py-4">
    ...
    <?php if ($resNuevos && $resNuevos->num_rows > 0): ?>
        <div class="mb-5">
            <div class="d-flex justify-content-between align-items-end mb-3 border-bottom border-light pb-2">
                <h3 class="fw-bold mb-0 text-dark"><i class="fas fa-bolt text-warning me-2"></i> Actualizaciones Recientes
                </h3>
            </div>
            ...
            <div class="d-flex gap-3 overflow-auto pt-2 pb-4 px-2 scroll-novedades">
                <?php while ($cap = $resNuevos->fetch_assoc()): ?>
                    ...
                    <div class="card-obra shadow-sm border-0 hover-effect bg-white d-flex flex-column flex-shrink-0 rounded-4 overflow-hidden"
                        style="width: 200px;">
                        <a href="/obra/<?php echo urlencode($cap['obra_slug']); ?>/<?php echo urlencode($cap['cap_slug']); ?>"
                            ...
                            <div class="portada-wrapper">
                                ...
                                <span
                                    class="position-absolute start-0 badge bg-dark bg-opacity-75 m-2 shadow-sm d-flex align-items-center z-3 rounded-pill px-2"
                                    style="top: 25px; font-size: 0.75rem;">
                                ...
                                <img src="<?php echo htmlspecialchars($imgPortadaNov); ?>"
                                    ...
                                    style="object-fit: cover; filter: brightness(0.95);" alt="Portada">
                            </div>
                            ...
                            <div class="card-body p-3 d-flex flex-column justify-content-between h-100 bg-white">
                                <div>
                                    ...
                                    <p class="text-iori small mb-2 text-truncate fw-bold d-inline-block px-2 py-1 rounded-2"
                                        style="background-color: rgba(10, 134, 136, 0.15); max-width: 100%;">
                                        <i class="far fa-file-alt me-1 opacity-75"></i>
                                        <?php echo htmlspecialchars($cap['cap_titulo']); ?>
                                    </p>
                                </div>
                                <small class="text-muted d-block mt-auto fw-semibold" style="font-size: 0.75rem;">
                                    <i
                                        class="far fa-clock me-1 text-iori"></i><?php echo tiempo_transcurrido($cap['fecha_subida']); ?>
                                </small>
                            </div>
                        </a>
                    </div>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-9">
            ...
            <div
                class="d-flex flex-wrap justify-content-between align-items-center mb-4 border-bottom border-light pb-3 gap-3">
                <h3 class="fw-bold text-dark m-0">
                    <?php
                    // Feedback visual Reactivo del filtro actual para el usuario
                    if (!empty($busqueda))
                        echo 'Búsqueda: <span class="text-iori">"' . htmlspecialchars($busqueda) . '"</span>';
                    elseif (!empty($filtro_genero))
                        echo 'Género: <span class="text-iori">' . htmlspecialchars($filtro_genero) . '</span>';
                    elseif (!empty($filtro_autor))
                        echo 'Autor: <span class="text-iori">' . htmlspecialchars($filtro_autor) . '</span>';
                    else
                ...
                <div class="d-flex gap-2">
                    <div class="dropdown">
                        <button
                            class="btn bg-light text-dark border shadow-sm rounded-pill btn-sm fw-bold px-3 dropdown-toggle hover-bg-light"
                            type="button" style="cursor: pointer;" data-bs-toggle="dropdown">
                    ...
                    <?php while ($obra = $resultado->fetch_assoc()): ?>
                        ...
                        <div class="col">
                            <a href="/obra/<?php echo urlencode($obra['slug']); ?>"
                                ...
                                <div
                                    class="card-obra h-100 shadow-sm border-0 hover-effect bg-white d-flex flex-column rounded-4 overflow-hidden">
                                    <div class="portada-wrapper">
                                        <div class="position-absolute w-100 text-center fw-bold text-white py-1 z-2"
                                            style="top: 0; left: 0; background-color: <?php echo $colorTipoCat; ?>; font-size: 0.75rem; letter-spacing: 1px;">
                                        ...
                                        <?php if ($demoObraCat !== 'DESCONOCIDO'): ?>
                                            <div class="position-absolute w-100 text-center fw-bold text-white py-1 z-2"
                                                style="bottom: 0; left: 0; background-color: <?php echo $colorDemoCat; ?>; font-size: 0.70rem; letter-spacing: 1px;">
                                    ...
                                    <div class="card-body p-3 d-flex flex-column justify-content-between">
                                        <div>
                                            ...
                                            <small class="text-muted d-block text-truncate mb-2" style="font-size:0.8rem;"><i
                                                    class="fas fa-pen-nib me-1 opacity-50"></i><?php echo htmlspecialchars($obra['autor']); ?></small>

                                            <div class="d-flex flex-wrap gap-1">
                                                <?php
                                                if (!empty($obra['generos'])) {
                                                    $generos_arr = explode(',', $obra['generos']);
                                                    // Usamos array_slice para limitar a 3 géneros máximo desde el backend
                                                    foreach (array_slice($generos_arr, 0, 3) as $g) {
                        ...
                        <ul class="pagination justify-content-center">
                            <li class="page-item <?php echo ($pagina_actual <= 1) ? 'disabled' : ''; ?>">
                                <a class="page-link shadow-sm border-0 fw-bold <?php echo ($pagina_actual <= 1) ? 'text-muted' : 'text-iori'; ?>"
                                    href="<?php echo $url_base . 'pagina=' . ($pagina_actual - 1); ?>"
                            ...
                            <?php for ($i = 1; $i <= $total_paginas; $i++): ?>
                                <li class="page-item <?php echo ($pagina_actual == $i) ? 'active' : ''; ?>">
                                    <a class="page-link shadow-sm border-0 fw-bold <?php echo ($pagina_actual == $i) ? 'bg-iori text-white' : 'text-dark'; ?>"
                                        href="<?php echo $url_base . 'pagina=' . $i; ?>" style="padding: 10px 16px;">
                                        <?php echo $i; ?>
                                    </a>
                                </li>
                            <?php endfor; ?>
        ...
        <div class="col-lg-3 d-none d-lg-block">
            ...
            <div class="card shadow-sm border-0 rounded-4 bg-white mb-4">
                <div class="card-header bg-white border-bottom-0 pt-4 pb-2">
                    <h6 class="fw-bold text-dark text-uppercase mb-0"><i class="fas fa-trophy text-warning me-2"></i>
                        Top Valoradas</h6>
                </div>
                        ...
                        if ($resTop && $resTop->num_rows > 0):
                            while ($top = $resTop->fetch_assoc()):
                                ...
                                <li class="list-group-item border-light py-3 px-4 hover-bg-light transition-colors">
                                    <a href="/obra/<?php echo urlencode($top['slug']); ?>"
                                        class="text-decoration-none d-flex align-items-center gap-3">

                                        <span class="fw-bold fs-5 <?php echo $colorRank; ?> flex-shrink-0 text-center"
                                            style="width: 28px;">#<?php echo $rank; ?></span>

                                        <img src="<?php echo htmlspecialchars($portadaTop); ?>"
                                            class="rounded shadow-sm flex-shrink-0" width="45" height="65"
                                            style="object-fit: cover;">

                                        <div class="overflow-hidden flex-grow-1" style="min-width: 0;">
                                            <h6 class="fw-bold text-dark text-truncate mb-1 hover-text-iori"
                                            ...
                                            <span
                                                ...
                                                <i
                                                    class="fas fa-star text-warning"></i><span><?php echo round($top['nota'], 1); ?></span>
                                            </span>
                                        </div>
                            ...
                            <li class="list-group-item text-center py-4 text-muted small">No hay suficientes valoraciones
                                ponderadas.</li>
                        <?php endif; ?>
                    </ul>
                </div>
            ...
            <div class="card shadow-sm border-0 rounded-4 bg-white">
                <div class="card-header bg-white border-bottom-0 pt-4 pb-2">
                    <h6 class="fw-bold text-dark text-uppercase mb-0"><i class="fas fa-comments text-iori me-2"></i>
                        Último en el Foro</h6>
                </div>
                <div class="card-body p-0">
                    <ul class="list-group list-group-flush">
                        <?php if ($resForo && $resForo->num_rows > 0):
                            while ($foro = $resForo->fetch_assoc()): ?>
                                <li class="list-group-item border-light py-3 px-4 hover-bg-light transition-colors">
                                    <a href="/foro/<?php echo urlencode($foro['slug']); ?>"
                                        class="text-decoration-none d-block">
                                        <h6 class="fw-bold text-dark text-truncate mb-2 hover-text-iori"
                                        ...
                                        <div class="d-flex justify-content-between align-items-center text-muted"
                                            style="font-size:0.75rem;">
                                            <span><i
                                                    class="far fa-clock me-1"></i><?php echo tiempo_transcurrido($foro['fecha']); ?></span>

                                            <span class="badge bg-light text-secondary border rounded-pill"
                                                title="Métricas de interacción">
                                                <i
                                                    class="fas fa-reply me-1 opacity-75"></i><?php echo $foro['num_respuestas']; ?>
                                            </span>
                                        </div>
                                    </a>
                                </li>
                    ...
                    <div class="p-3 text-center border-top border-light bg-light rounded-bottom-4">
                        <a href="/foro"
                            class="btn btn-sm btn-outline-iori fw-bold text-uppercase rounded-pill px-4 shadow-sm w-100">
