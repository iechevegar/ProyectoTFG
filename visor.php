...
if (!isset($_GET['obraSlug']) || !isset($_GET['capSlug'])) die("Error: URL incompleta.");
...
if ($userId && isset($_SESSION['rol']) && $_SESSION['rol'] !== 'admin') {
...
<body class="visor-body">
    ...
    <div class="visor-barra">
        ...
        <div style="display:flex; gap: 0.5rem;">
            ...
            <?php if($slugPrev): ?>
                <a href="/obra/<?php echo $obraSlug; ?>/<?php echo $slugPrev . $queryAdmin; ?>" class="btn btn-sm btn-secondary btn-mini-nav"><i class="fas fa-chevron-left"></i></a>
            <?php else: ?>
                <button class="btn btn-sm btn-secondary btn-mini-nav" disabled style="opacity:0.3"><i class="fas fa-chevron-left"></i></button>
            <?php endif; ?>

            <?php if($slugNext): ?>
                <a href="/obra/<?php echo $obraSlug; ?>/<?php echo $slugNext . $queryAdmin; ?>" class="btn btn-sm btn-primary btn-mini-nav"><i class="fas fa-chevron-right"></i></a>
            <?php else: ?>
    ...
    <div class="visor-contenido" id="contenedor-imagenes">
        <?php foreach ($lista_imagenes as $url): ?>
            <?php 
                // TRUCO: Si es un link externo (http) lo deja igual, si es interno le pone la barra /
                $imgSrc = (strpos($url, 'http') === 0) ? $url : '/' . ltrim($url, '/'); 
            ?>
            <img src="<?php echo htmlspecialchars($imgSrc); ?>" class="pagina-manga" loading="lazy" alt="Página">
        <?php endforeach; ?>
    ...
    <div class="navegacion-capitulos">
        <?php if($slugPrev): ?>
            <a href="/obra/<?php echo $obraSlug; ?>/<?php echo $slugPrev . $queryAdmin; ?>" class="btn-nav-cap btn-anterior"><i class="fas fa-arrow-left"></i> Anterior</a>
        <?php else: ?>
            <div class="btn-nav-cap btn-disabled">Primer Capítulo</div>
        <?php endif; ?>

        <?php if($slugNext): ?>
            <a href="/obra/<?php echo $obraSlug; ?>/<?php echo $slugNext . $queryAdmin; ?>" class="btn-nav-cap btn-siguiente">Siguiente <i class="fas fa-arrow-right"></i></a>
        <?php else: ?>
    ...
    <div class="zona-comentarios">
        ...
        <?php if(isset($_SESSION['usuario'])): ?>
            ...
            <?php if($estaSuspendido): ?>
                <div class="alert text-center mb-5 py-4" style="background-color: #2a1111; border: 1px solid #ff4444; color: #ff8888; border-radius: 8px;">
                    ...
                    <small class="fw-bold d-block mt-2" style="color: #ffaaaa;">Podrás volver a comentar el: <?php echo $fechaDesbloqueoStr; ?></small>
                </div>
            <?php else: ?>
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <span class="fecha-comentario">
                                ...
                                <?php if(isset($_SESSION['rol']) && $_SESSION['rol'] === 'admin'): ?>
                                    <form method="POST" class="d-inline ms-2" onsubmit="return confirm('¿Borrar este comentario?');">
