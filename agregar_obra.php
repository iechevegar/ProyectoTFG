...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Si todo va bien, insertamos en la BD añadiendo el SLUG
    if (empty($mensaje)) {
        $sql = "INSERT INTO obras (titulo, slug, autor, generos, sinopsis, portada) 
                VALUES ('$titulo', '$slug', '$autor', '$generos', '$sinopsis', '$ruta_portada')";
        ...
        <div class="col-md-8">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <h2 class="fw-bold text-dark m-0"><i class="fas fa-book-medical text-success me-2"></i> Añadir Nueva Obra</h2>
                </div>
            </div>
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="titulo" class="form-control bg-light" required placeholder="Ej: Solo Leveling">
                            </div>
                            
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-bold text-secondary small text-uppercase">Autor(es) *</label>
                                <input type="text" name="autor" class="form-control bg-light" required placeholder="Ej: Chugong">
                            </div>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold text-secondary small text-uppercase">Géneros *</label>
                            <input type="text" name="generos" class="form-control bg-light" required placeholder="Ej: Acción, Fantasía, Aventura (separados por coma)">
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold text-secondary small text-uppercase">Sinopsis</label>
                            <textarea name="sinopsis" class="form-control bg-light" rows="5" placeholder="Escribe un resumen de la trama..."></textarea>
                        </div>

                        <div class="mb-4 p-3 bg-light rounded border">
                            <label class="form-label fw-bold text-primary small text-uppercase"><i class="fas fa-file-upload me-1"></i> Subir Portada (JPG, PNG) *</label>
                            <input type="file" name="portada" class="form-control" accept="image/*" required>
                            <div class="form-text mt-2"><i class="fas fa-info-circle me-1"></i> La imagen se subirá directamente a la carpeta del servidor.</div>
                        </div>
